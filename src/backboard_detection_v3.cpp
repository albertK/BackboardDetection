...
void edge_detection(cv::Mat img_in, cv::Mat& img_out)
{    
...
int main(int argc, char** argv)
{
    ...
    cv::Mat edge_img;
    edge_detection(hole_filled_img, edge_img);
    cv::namedWindow("Backboard edge", CV_WINDOW_NORMAL);
    cv::imshow("Backboard edge", edge_img);
    cv::waitKey();
    cv::destroyWindow("Backboard edge");
    
    //Image line detect
    std::vector<cv::Vec4i> lines;
    cv::Mat line_img;
    cv::cvtColor(edge_img, line_img, CV_GRAY2BGR);
    cv::HoughLinesP(edge_img, lines, 5, CV_PI/50, 80, 80, 60 );
