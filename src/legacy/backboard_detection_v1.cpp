...
void plane_projection(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in, pcl::ModelCoefficients::Ptr plane_coeff,cv::Mat image_out, Eigen::Matrix4d& transform)
{
	...
	//cv::Mat img = cv::Mat::zeros(img_h, img_w, CV_8UC1);
	//printf("Image size = %d x %d\n", image_out.rows, image_out.cols);
	image_out.create(img_h, img_w, CV_8UC1);
	for(unsigned int i = 0; i < transformed_plane->size(); ++i)
	{
	    int col = (int)ceil(transformed_plane->points[i].x);
	    int row = (int)ceil(transformed_plane->points[i].y);
