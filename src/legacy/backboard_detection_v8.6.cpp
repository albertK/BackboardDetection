...
int main(int argc, char** argv)
{
...
#if KINECT
    //初始化OpenNI device
    OpenNIFrameSource::OpenNIFrameSourceAlbert camera;
    sleep(3);
	...
	pcl::ScopeTime frame_rate("Backboard detection takes");
        ...
        float iteration = 10;
	float theta_err_avg = 0;
	float distance_avg = 0;
	float location_x_avg = 0;
	float location_y_avg = 0;
	float orientation_angle_avg = 0;
	    ...
	    h=0.9;
            ...
            backboard_align(cloud_border_model, cloud_border, aligned_model, result_transform);
	    ...
	    theta_err_avg += (theta_err/iteration);
#if DBG
            printf("Offset angle = %f deg\n", theta_err);
#endif
	    
            //calculate the distance between the robot and the center of the basket
	    float distance = sqrt(pow(backboard_center(0), 2.0) + pow(backboard_center(2), 2.0));
	    distance_avg += distance/iteration;
#if DBG
	    printf("Distance = %f\n", distance);
#endif
	    ...
	    orientation_angle_avg += orientation_angle/iteration/3.14*180;
	    location_x_avg += location_x/iteration;
	    location_y_avg += location_y/iteration;
#if DBG
	    printf("Location = (%f, %f)\n", location_x, location_y);
	    printf("Orientation = %f deg\n", orientation_angle/3.14*180);
#endif
        ...
        printf("offset angle avg = %f\n", theta_err_avg);
	printf("distance avg = %f\n", distance_avg);
	printf("location_x avg = %f\n", location_x_avg);
	printf("location_y avg = %f\n", location_y_avg);
	printf("orientation_angle avg = %f\n", orientation_angle_avg);
        ...
        if(recv_msg[0] == 'a')
        {
            ...
            if(theta_err_avg >= 0)
                ...
                send_buff[0] = 1;
            send_buff[1] = abs((int)round(theta_err_avg));
        ...
        {
            unsigned char send_buff[2];
            send_buff[0] = (int)distance_avg;
            send_buff[1] = ((int)(distance_avg*100))%100;
            send(new_socket, &send_buff, 2, 0);
        ...
        {
            unsigned char send_buff[2];
            send_buff[0] = (int)location_x_avg;
            send_buff[1] = ((int)(location_x_avg*100))%100;
            send(new_socket, &send_buff, 2, 0);
        ...
        {
            unsigned char send_buff[2];
            send_buff[0] = (int)location_y_avg;
            send_buff[1] = ((int)(location_y_avg*100))%100;
            send(new_socket, &send_buff, 2, 0);
            printf("I send location_y = (%d, %d)\n", send_buff[0], send_buff[1]);
        }
        else if(recv_msg[0] == 'o')
	{
	    unsigned char send_buff[2];
	    if(orientation_angle_avg >= 0)
		send_buff[0] = 0;
	    else
		send_buff[0] = 1;
	    send_buff[1] = abs((int)round(orientation_angle_avg));
	    send(new_socket, &send_buff, 2, 0);
	    printf("I send orientation = (%d, %d)\n", send_buff[0], send_buff[1]);
	}
        else if(recv_msg[0] == 'c')
        {
#if KINECT
